...
if(isset($_GET['u']) && is_numeric($_GET['u'])){
	$id=$_GET['u'];
	$stat="SELECT realname, name, email, dl_count, city_count from customers WHERE id=".quote_smart($id);
...
<input type="text" name="u_cityc" value="$row[4]" size="3"/> city</p>
...
if(!isset($_POST['cancel']) && isset($_POST['u_id']) && is_numeric($_POST['u_id'])){
	...
	if(mysql_query($stat)){
		...
		if(strcmp($pwd1, $pwd2)==0 && strlen($pwd1)==9){
			...
			echo "$stat<br/>";
			if(!mysql_query($stat)){
				echo "Error when setting password:<br/>".mysql_error();
			}
		}
		else{
		...
		echo "Error when updating user:<br/>".mysql_error();
	}
}
?>
...
<?php
$stat="SELECT realname, email, dl_count, city_count, id from customers ORDER BY realname";
$sth=mysql_query($stat);
$i=0;
